...
public class AppleUrlManager {
	...
	public ImageButtonData[] getNavigationData() {
		...
		if (document != null) {
			...
			for (Element listItem : listItems) {
				...
				ImageButtonData data = new ImageButtonData(link.attr("href"),
						image.attr("src"),
						Integer.parseInt(image.attr("width")),
						Integer.parseInt(image.attr("height")), header.text());
				buttonData[i] = data;
				i++;
			}
	...
	public ProductData[] getProductData(String url) {
		...
		if (document != null) {
			...
			for (Element row : rows) {
				productData[i] = extractProductData(row);
				i++;
			}
		}
	...
	private ProductData extractProductData(Element row) {
		Element image = row.select("td.image img").first();
		...
		Element price = priceParent.getElementsByAttributeValue("itemprop",
				"price").first();
		Element savings = row.select("td.purchase-info p.savings").first();
		ProductData data = new ProductData(image.attr("src"),
				Integer.parseInt(image.attr("width")), Integer.parseInt(image
						.attr("height")), header.text(), specs.toString(),
				price.html(), savings == null ? null : savings.html());
		return data;
	}
	...
	private void setBaseUrl(String url) {
		...
		baseUrl = String.format("%s://%s", fullUri.getScheme(),
				fullUri.getHost());
